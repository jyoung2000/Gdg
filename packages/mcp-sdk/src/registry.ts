...
export function curatedCatalog(): CatalogEntry[] {
  ...
  return [
    ...
    mk(
      'github',
      'GitHub (official)',
      'GitHub\'s official MCP server: repositories, issues, pull requests, actions and code search as tools. The hosted endpoint needs a GitHub PAT in the Authorization header; the container form runs locally with the same token.',
      'version-control',
      [
        {
          kind: 'http',
          ...
          display: 'Connect to https://api.githubcopilot.com/mcp/ (Authorization: Bearer <your GitHub PAT>)',
          note: 'Hosted by GitHub; requires outbound access and a PAT with the scopes you want to grant',
        },
        {
          kind: 'docker',
          transport: 'stdio',
          ...
          url: null,
          display: 'docker run --rm -i -e GITHUB_PERSONAL_ACCESS_TOKEN ghcr.io/github/github-mcp-server',
          note: 'Runs locally; needs Docker and ghcr.io egress to pull the image',
        },
      ],
      [{ name: 'GITHUB_PERSONAL_ACCESS_TOKEN', description: 'GitHub personal access token; grant only the scopes you intend to use', secret: true }],
      'development',
